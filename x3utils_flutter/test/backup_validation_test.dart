...
void main() {
  ...
  group('Firmware.inspectDumpBytes', () {
    test('a full varied in-memory image is a backup', () {
      final c = Firmware.inspectDumpBytes(_image(Firmware.expectedSize));
      expect(c.verdict, DumpVerdict.ok);
      expect(c.ok, isTrue);
    });

    test('a short in-memory read is rejected', () {
      final c = Firmware.inspectDumpBytes(_image(32768));
      expect(c.verdict, DumpVerdict.incomplete);
      expect(c.message, contains('32768 of 131072'));
    });

    test('a full-size masked read remains a protection finding', () {
      final c = Firmware.inspectDumpBytes(
        _image(Firmware.expectedSize, fill: 0x00),
      );
      expect(c.verdict, DumpVerdict.masked);
      expect(c.isEvidence, isTrue);
    });
