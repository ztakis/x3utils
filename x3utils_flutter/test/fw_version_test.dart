...
void main() {
  ...
  group('real corpus (skipped when the private mirror is absent)', () {
    // set X3U_FW_MIRROR to a local firmware corpus root to enable
    final mirror = Platform.environment['X3U_FW_MIRROR'];
    ...
    for (final entry in cases.entries) {
      ...
      test('identifies $model $type $expected', () {
        if (mirror == null) {
          markTestSkipped('X3U_FW_MIRROR not set');
          return;
        }
        final f = File('$mirror\\${entry.key}');
        if (!f.existsSync()) {
          markTestSkipped('file not present: ${f.path}');
          return;
        }
