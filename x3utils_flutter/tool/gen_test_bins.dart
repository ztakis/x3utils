...
Uint8List payload(
  ...
  switch (key) {
    ...
    case KeyState.shu:
      for (var i = 0; i < 16; i++) {
        b[kKeyOff + i] =
            int.parse(kShuKeyHex.substring(i * 2, i * 2 + 2), radix: 16);
      }
      b.fillRange(kRandOff, kRandOff + 6, 0xFF);
    case KeyState.oemStyle:
      // Real OEM keys: 16 lowercase alnum; rand: 6 mixed-case alnum.
      b.setAll(kKeyOff, asciiBytes(rng.text('abcdefghijklmnopqrstuvwxyz0123456789', 16)));
      b.setAll(kRandOff, asciiBytes(rng.text('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789', 6)));
  }
...
List<MapEntry<String, Uint8List>> readZipEntries(Uint8List zipBytes) {
  ...
  return [
    for (final f in a.files)
      if (f.isFile) MapEntry(f.name, Uint8List.fromList(f.content as List<int>)),
  ];
}
...
void emit(String relName, Uint8List bytes, String group, String knob,
    String expected) {
...
void copyReal(Directory corpusRoot, String srcRel, String destName,
    String knob, String expected) {
  final src = File(
...
void main(List<String> args) {
  if (args.isEmpty) {
    stderr.writeln('Usage: dart run tool/gen_test_bins.dart <out_dir> [known_good_zip3]');
    exit(2);
  }
  ...
  final zeros = Uint8List(kFullSize); // all 0x00, no marker: content IS the knob
  emit('1_all_zer{o}s.bin', zeros, 'degenerate', 'braces in path',
      'reject unsafe path before OpenOCD starts');
  emit('2_all_zeros.bin', zeros, 'degenerate', 'single repeated byte',
      'reject as repeated-content image');
  emit('3_synthetic_64k.bin', payload('3_synthetic_64k', banner: null, size: 65536),
      'slot', 'no banner (64 KiB)',
  ...
  emit('4b_zt3_vcu_SYNTHETIC_FULL.bin', fullImage(payload('4b')),
      'full', 'accept baseline (zt3 VCU)',
      'identity accept vs zt3 VCU target; fake runner may reach the write command');
  emit('4c_gt3_vcu_SYNTHETIC_FULL.bin',
      fullImage(payload('4c', banner: kBannerGt3Vcu), serial: kSerialGt3),
      'full', 'banner model = gt3', 'identity reject: cross-model VCU');
  emit('4d_zt3_mcu_SYNTHETIC_FULL.bin',
      fullImage(payload('4d', banner: kBannerMcu), mcu: true, serial: kSerialMcuPart),
      'full', 'banner type = MCU', 'identity reject: VCU/MCU mismatch');
  emit('5_mutation_source_zt3_vcu_SYNTHETIC_FULL.bin', fullImage(payload('5')),
      'full', 'protected mutation source',
  ...
  emit('11a_zt3_vcu_key_shu_SYNTHETIC_FULL.bin',
      fullImage(payload('11a', key: KeyState.shu)),
      'full', 'key @0x1420 = SHU default',
      'Make zip3 accepts (defaultKey branch)');
  emit('11b_zt3_vcu_key_oemstyle_SYNTHETIC_FULL.bin',
      fullImage(payload('11b', key: KeyState.oemStyle)),
      'full', 'key/rand @0x1420 = ASCII production-style',
  ...
  emit('13a_zt3_vcu_banner_unknown_code_SYNTHETIC_FULL.bin',
      fullImage(payload('13a', banner: kBannerUnknown)),
      'full', 'banner code xxZ9 (shape-valid, unsupported)',
      'guarded Backup+Flash rejects unsupported banner');
  emit('13b_zt3_vcu_no_banner_SYNTHETIC_FULL.bin',
  ...
  emit('14a_slot_zt3_vcu_SYNTHETIC.bin', payload('14a'),
      'slot', 'accept baseline ($kPayloadLen B)',
      'guarded Flash slot 0 accepts');
  emit('14b_slot_too_small_SYNTHETIC.bin', payload('14b', size: 40964),
      'slot', 'size 40964 < window min', 'reject: too small for slot 0');
  emit('14c_slot_too_big_SYNTHETIC.bin', payload('14c', size: 71684),
      'slot', 'size 71684 > window max', 'reject: too big for slot 0');
  emit('14d_slot_edge_64k_SYNTHETIC.bin', payload('14d', size: 65536),
      'slot', 'size exactly 65536 (window edge)',
  ...
  if (!zipSrc.existsSync()) {
    ...
    void emitZip(String name, List<MapEntry<String, Uint8List>> entries,
            String knob, String expected) =>
        emit(name, writeZipEntries(entries), 'zip3', knob, expected);

    emitZip('15a_zip3_schema2.zip',
        editInfo(srcEntries, (m) => m['schemaVersion'] = 2),
        'schemaVersion = 2', 'reject: unsupported schemaVersion');
    emitZip('15b_zip3_no_infojson.zip',
        [for (final e in srcEntries) if (e.key != 'info.json') e],
        'info.json removed', 'reject: not a v3 package');
    emitZip('15c_zip3_bad_json.zip',
        [for (final e in srcEntries)
          e.key == 'info.json'
              ? MapEntry(e.key, asciiBytes('{ this is not json'))
              : e],
        'info.json is garbage text', 'reject: info.json is not valid JSON');
    emitZip('15d_zip3_no_firmware_record.zip',
        editInfo(srcEntries, (m) => m.remove('firmware')),
        'firmware record removed', 'reject: no firmware record');
    emitZip('15e_zip3_no_model.zip',
        editInfo(srcEntries, (m) => (m['firmware'] as Map).remove('model')),
        'firmware.model removed', 'reject: no firmware.model');
    emitZip('15f_zip3_unsupported_model.zip',
        editInfo(srcEntries, (m) => (m['firmware'] as Map)['model'] = 'm365'),
        'model = m365', 'reject: unsupported model');
    emitZip('15g_zip3_no_md5enc.zip',
        editInfo(srcEntries,
            (m) => ((m['firmware'] as Map)['md5'] as Map).remove('enc')),
        'md5.enc removed', 'reject: package is not MD5-verified');
    emitZip('15h_zip3_md5_mismatch.zip',
        [for (final e in srcEntries)
          e.key == 'FIRM.bin.enc'
              ? MapEntry(e.key,
                  Uint8List.fromList(e.value)..last ^= 0xFF)
              : e],
        'one byte flipped in FIRM.bin.enc', 'reject: failed MD5 check');
    emitZip('15i_zip3_model_mismatch.zip',
        editInfo(srcEntries, (m) => (m['firmware'] as Map)['model'] = 'g3'),
        'model relabeled g3 (payload banner stays zt3)',
        'reject: banner/metadata mismatch (MD5 still valid)');
    final pad = Uint8List(20480);
    Rng(seedOf('15j_pad')).fill(pad);
    emitZip('15j_zip3_oversize.zip',
    ...
    emit('15k_notazip.zip', junk, 'zip3', 'random bytes named .zip',
        'reject: not a readable ZIP archive');
  }

  // ── 16: path / extension / size gates ─────────────────────────────────────
  stdout.writeln('Path and size gates:');
  emit('Prüfung${sep}16a_slot_zt3_vcu_SYNTHETIC.bin', payload('14a'),
      'path', 'non-ASCII (German umlaut) directory in path',
      'reject non-ASCII path before OpenOCD starts');
  emit('16b_slot_zt3_vcu_SYNTHETIC.dat', payload('14a'),
      'path', 'extension .dat', 'reject: only .bin is allowed');
  final trunc = Uint8List(100004);
  Rng(seedOf('16c')).fill(trunc);
  trunc.setAll(kSlot0Off + kBannerOff, asciiBytes(kBannerZt3Vcu));
  emit('16c_truncated_full_SYNTHETIC.bin', trunc,
      'full', 'full image truncated to 100004 B',
      'reject: invalid size for a full image');

  // ── 17: real files promoted from the corpus ───────────────────────────────
  stdout.writeln('Real-file copies:');
  copyReal(corpusRoot, 'repo/g3/VCU/FIRM_1.4.8 (Compat).bin',
  ...
  copyReal(corpusRoot, 'full_dumps/MEMORY_ZT3Pro.bin',
  ...
  final csv = StringBuffer('﻿File,Group,Knob,ExpectedVerdict,SizeBytes,Sha256\n');
  for (final row in manifest) {
    csv.writeln(row.map((c) => '"${c.replaceAll('"', '""')}"').join(','));
  }
  File('${outDir.path}${sep}gen_manifest.csv').writeAsStringSync(csv.toString());
  stdout.writeln('\n${manifest.length} files written to ${outDir.path}');
  stdout.writeln('Manifest: gen_manifest.csv (file, knob turned, expected verdict, sha256)');
}
