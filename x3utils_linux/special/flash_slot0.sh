...
if [[ "${RACE:-false}" == "true" ]]; then
    echo
    echo "$D"
    echo -e "   ${CL_M}Power-race slot0 flash (mode D) - backup, then flash slot0 (2 catches)${CL_NC}"
    echo "$D"
    ...
    echo -e "   NOT replug mid-flash; write_image re-erases, so a retry is safe). ${CL_C}Ctrl+C to stop.${CL_NC}"
