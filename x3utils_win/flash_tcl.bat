...
:: Strip any accidental outer quotes and resolve full path
for /f "delims=" %%A in ("%bin_file_path%") do set "bin_file_path=%%~fA"
for /f "delims=" %%A in ("%bin_file_path%") do set "extension=%%~xA"
...
:: Reject curly braces in user-supplied filename
for %%C in ("{" "}") do (
