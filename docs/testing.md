...
| 2026-07-31 | Linux Mint home primary / x86_64 | AT32F415 X3 testbed | ST-LINK | A / Default SWD | Packaged AppImage v1.2.3 BETA5 — RDP single-log Check | pass | The rebuilt `dist/x3utils-1.2.3-x86_64.AppImage` was built at 14:33 from the clean `41d861d` checkout and Check protection ran at 14:34. The GUI passed `bash rdp_check.sh --launcher --no-toolkit-log` and created exactly one new file, `~/x3utils/logs/rdp_check/rdp_check_2026-07-31_14-34-11.log` (1392 B). No same-run `rdp_check_toolkit_*` companion appeared; the three July 29 toolkit logs remained visible and were the only toolkit logs in the supplied directory evidence. The complete GUI log recorded USD `ffff5aa5`, consistent FAP `0xA5`/comp `0x5A`, readable main flash (MSP `0x20000550`, reset vector `0x08000121`), `NOT PROTECTED`, and exit 0. No retry occurred. This closes packaged-AppImage Linux coverage for the BETA5 single-log change; a live BETA5 retry and a real direct-script run without the flag remain separate. |
| 2026-07-31 | macOS 15.7.7 / Intel x86_64 | ZT3 VCU test board | ST-LINK | A / Default SWD | Packaged GUI v1.2.3 BETA5 — embedded backend and RDP single-log Check | pass | The app ran from `dist/x3utils-1.2.3-macos-universal/x3utils.app`. Check connection resolved the embedded backend at `Contents/MacOS/native/macos/oocd/scripts`, halted the target, found the `artery` flash bank, exited 0, and saved `check_2026-07-31_14-22-45.log` (745 B). Check protection then passed `bash rdp_check.sh --launcher --no-toolkit-log`, created exactly one new GUI log, `rdp_check_2026-07-31_14-22-52.log` (1445 B), and created no new `rdp_check_toolkit_*` companion; only the two old July 29 toolkit logs remain in the directory. The complete log recorded USD `ffff5aa5`, consistent FAP `0xA5`/comp `0x5A`, readable main flash (MSP `0x20000550`, reset vector `0x08000121`), `NOT PROTECTED`, and exit 0. No retry occurred. This closes packaged-app macOS coverage for the BETA5 single-log change; a real direct-script run without the flag remains separate. |
| 2026-07-31 | macOS 15.7.7 / Intel x86_64 | ZT3 VCU test board | ST-LINK | A / Default SWD | GUI v1.2.3 BETA5 debug/source-build RDP single-log Check | pass (source-build hardware); packaged pass recorded separately above | The GUI command was `bash rdp_check.sh --launcher --no-toolkit-log`. Exactly one new file appeared in `~/x3utils/logs/rdp_check/`, `rdp_check_2026-07-31_14-16-48.log` (1445 B), and no same-run `rdp_check_toolkit_*` companion appeared; the two older July 29 GUI/toolkit pairs were left in place and visible in the supplied before/after directory evidence. The complete GUI log recorded `ffff5aa5`, consistent FAP `0xA5`/comp `0x5A`, readable main flash (MSP `0x20000550`, reset vector `0x08000121`), `NOT PROTECTED`, and exit 0. No retry occurred. On the same macOS checkout at commit `0fc483a`, `rdp_runner_test.dart` passed 6/6 and `flutter analyze` found no issues. This closes the debug/source-build macOS handoff; packaged-app coverage is recorded separately above, while a real direct-script compatibility run remains separate. |
...
- On each platform, run Check protection with Save log enabled and confirm the
  one complete console log lands at
  `<root>/logs/rdp_check/rdp_check_<stamp>.log`. No
  `rdp_check_toolkit_*` may appear from a GUI run; on Windows, runtime
  `config.cmd` must not appear either. Direct script runs without the GUI-only
  suppression flag keep their toolkit transcript. Nothing may appear under
...
| --- | --- | --- | --- | --- |
| 2026-07-31 | Windows 11 development host | Flutter v1.2.3 BETA5 Unix GUI RDP single-log implementation | pass (static and Windows-hosted checks); macOS and Linux runtime subsequently passed | Unix GUI Check now passes `--no-toolkit-log`; the bundled Linux/macOS `rdp_check.sh` copies retain their temporary per-attempt files for parsing and live retry output but skip the persistent `_toolkit` file and its `Log file:`/`Full log:` claims. Unix Rescue is unchanged because it never created a toolkit transcript. A direct script run without the flag retains the existing toolkit file. `rdp_runner_test.dart` adds target-platform coverage for GUI suppression, the complete `NOT PROTECTED` verdict, no log directory from the runner-only fixture, direct-script compatibility, and retry output; on this Windows host the file reported 6/6 but only its two Windows cases executed, so that was not Linux/macOS runtime evidence. Both edited shell scripts passed `bash -n` under Git for Windows. The BETA5 stage/root gate passed 20/20, the full Flutter suite passed 198/198, and `flutter analyze` was clean. Standalone `x3utils_linux`/`x3utils_mac`, Unix Rescue, temp `config.sh`, Tcl and packaging scripts are unchanged. The macOS debug and packaged passes and the packaged Linux AppImage pass are now recorded in the hardware table above. A live BETA5 retry and a real direct-script run without the flag remain separate compatibility evidence. |
